...
//                BSP_LEDOn ( APP_USB_LED_3 );
                LATAbits.LATA4 = 1; // whatever the LED pin is. 
...
//                    appData.yCoordinate = 0;
                    ...
                    //readIMU value here. and assign it xcoordinate. 
                    readIMU(OUT_TEMP_L, data, ARRLEN);
                    int i=0;
                    float valx = 0.0, valy =0.0;
                    for(i=0; i<ARRLEN/2; i++)
                    {
                        dataS[i] = (data[2*i+1] << 8 | data[2*i]);
                    }
                    valx = (((float)dataS[4]*2)/65535.0)*5.0; //-5g to 5g
                    valy = (((float)dataS[5]*2)/65535.0)*5.0; // -5g to 5g 
                    appData.xCoordinate = (int8_t)valx;
                    appData.yCoordinate = (int8_t)valy;
                    vector ++;
                    movement_length = 0;
                }
            }
