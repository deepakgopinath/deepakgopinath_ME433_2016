...
void LCD_displayCharacter(int x, int y, char c){
    if (x >= 0 && x < 123 && y >= 0 && y < 120) // each character is 5 pixels wide and 8 pixels deep
    {
        int i=0;
        char byte;
        for(i=0; i<5; i++)
        {
            ...
            for(j=7; j>=0; j--)
            {
                ...
                if(b == 1)
                {
                    LCD_drawPixel((short)(x+i), (short)(y+j), BLACK);
                }
                if(b == 0)
                {
                    LCD_drawPixel((short)(x+i), (short)(y+j), WHITE);
                }
            }
        }
    }
...
void LCD_displayString(int x, int y, char *str)
{
    if (x >= 0 && x < 123 && y >= 0 && y < 120) // make sure the top left of the string is at least within range
    {
        int i = 0;
        while(str[i]){ // while the character is not null
            LCD_displayCharacter(x+5*i, y, str[i]); // 
            i++;
        }
    }
}